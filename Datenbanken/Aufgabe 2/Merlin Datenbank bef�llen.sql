...
create table Beobachtungsort
  ...
  Beobachtungsort       varchar2(500),
  Staat                 varchar2(40),
  Zoographische_Region  varchar2(50) not null
  );
...
UPDATE VOEGEL
  ...
  where (select MERLIN.BIRDS_DE.de_deutsch
    from MERLIN.BIRDS_DE where latainischer_name = MERLIN.BIRDS_DE.de_latein) is not null;
...
INSERT INTO BEOBACHTUNGSORT (beobachtungsortid, zoographische_region) VALUES (1,'Paläarktis'); 
...
SELECT
  vogelid, 
  1
FROM voegel v, merlin.birds_de mbde where v.latainischer_name = mbde.de_latein;
